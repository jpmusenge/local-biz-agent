#!/usr/bin/env tsx
/**
 * Test Restaurant Premium Website Generation
 * Generates a premium restaurant website using Claude (or Gemini with --provider=gemini)
 * and opens it in the browser for review.
 *
 * Usage:
 *   npm run test:restaurant-premium
 *   npm run test:restaurant-premium -- --provider=gemini
 *   npm run test:restaurant-premium -- --mock          # skip API, use ClaudeClient mock
 */
...
import { buildRestaurantPremiumPrompt, checkWebsiteQuality } from '../src/modules/generator/templates/restaurant-premium.js';
...
function toBusinessInfo(row: Record<string, unknown>): BusinessInfo {
  return {
    ...
    businessType: String(row['business_type'] ?? row['category'] ?? 'restaurant'),
...
/** Find a restaurant from the DB, or return mock data if none exist. */
function getRestaurantBusiness(): BusinessInfo {
  try {
    db.initialize();
    const raw = db.raw();
    const row = raw.prepare(`
      SELECT * FROM businesses
      WHERE (LOWER(category) LIKE '%restaurant%'
          OR LOWER(business_type) LIKE '%restaurant%'
          OR LOWER(category) LIKE '%food%'
          OR LOWER(business_type) LIKE '%food%'
          OR LOWER(category) LIKE '%cafe%'
          OR LOWER(business_type) LIKE '%cafe%')
    ...
    if (row) {
      ...
      console.log(`Using DB restaurant: ${biz.name} (${biz.city}, ${biz.state})`);
      return biz;
    }
  } catch {
    // DB not initialised / no data — fall through to mock
  } finally {
    try { db.close(); } catch { /* ignore */ }
  }

  console.log('No restaurants in DB — using mock data.');
  return {
    ...
    state:        'MS',
    phone:        '(662) 252-0184',
    email:        'hello@magnoliatablehs.com',
    address:      '103 Van Dorn Ave, Holly Springs, MS 38635',
  };
}
...
async function main(): Promise<void> {
  ...
  console.log('RESTAURANT PREMIUM — Website Generator Test');
  console.log('='.repeat(60));
  console.log('');

  const business = getRestaurantBusiness();
  ...
  if (useMock) {
    // Use ClaudeClient mock mode (returns generic Tailwind template, not the premium prompt)
    ...
    // Gemini goes through buildWebsitePrompt which now routes restaurants to premium
    html = await client.generateWebsite(business, WebsiteTemplate.SUSPENDED_DARK);
  } else {
    const { ClaudeClient } = await import('../src/modules/generator/claude-client.js');
    const client = new ClaudeClient();
    if (client.isInMockMode()) {
      ...
      console.log('Use --mock to skip this check, or set ANTHROPIC_API_KEY in .env.\n');
    }
    // Claude goes through buildWebsitePrompt which now routes restaurants to premium
    html = await client.generateWebsite(business, WebsiteTemplate.SUSPENDED_DARK);
  }
  ...
  const filename = 'restaurant-premium-preview.html';
  ...
  // ── Feature checks ────────────────────────────────────────────────────────
  const checks = [
    { label: 'Cormorant Garamond font',  pattern: 'Cormorant+Garamond' },
